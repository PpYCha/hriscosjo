<?php
//Include Common Files @1-780B1087
define("RelativePath", ".");
define("PathToCurrentPage", "/");
define("FileName", "Query_SoloParent.php");
...
//employee_departmentoffice ReportGroup class @2-6BEF9626
...
//employee_departmentoffice GroupsCollection class @2-3E78D2F3
...
//Class_Initialize Event @2-E6F96B65
...
//Show Method @2-9F7E9CCF
    function Show()
    {
        ...
        $this->DataSource->Parameters["urls_SoloParent"] = CCGetFromGet("s_SoloParent", NULL);
...
//DataSource Variables @2-5045B7DF
    var $Parent = "";
    var $CCSEvents = "";
    var $CCSEventResult;
    var $ErrorBlock;
...
//DataSourceClass_Initialize Event @2-51C3FE02
...
//Prepare Method @2-35D9CA22
    function Prepare()
    {
        ...
        $this->wp->AddParameter("2", "urls_SoloParent", ccsText, "", "", $this->Parameters["urls_SoloParent"], "", false);
        ...
        $this->wp->Criterion[2] = $this->wp->Operation(opContains, "SoloParent", $this->wp->GetDBValue("2"), $this->ToSQL($this->wp->GetDBValue("2"), ccsText),false);
...
//Open Method @2-CD25EB2B
    function Open()
    {
        $this->CCSEventResult = CCGetEvent($this->CCSEvents, "BeforeBuildSelect", $this->Parent);
        $this->SQL = "SELECT Surname, FirstName, MiddleName, SoloParent, SoloParentDetails, employee.OfficeID AS employee_OfficeID, OfficeAcronym, NameOfficeDept \n\n" .
...
//SetValues Method @2-470F1A61
    function SetValues()
    {
        $this->OfficeAcronym->SetDBValue($this->f("NameOfficeDept"));
...
class clsRecorddepartmentoffice_employee { //departmentoffice_employee Class @14-B39F2074

//Variables @14-D6FF3E86
...
//Class_Initialize Event @14-4B61B6D5
...
"FROM departmentoffice {SQL_Where} {SQL_OrderBy}";
            ...
            $this->s_employee_OfficeID->DataSource->Order = "OfficeAcronym";
            $this->s_SoloParent = & new clsControl(ccsListBox, "s_SoloParent", "s_SoloParent", ccsText, "", CCGetRequestParam("s_SoloParent", $Method, NULL), $this);
            $this->s_SoloParent->DSType = dsTable;
            $this->s_SoloParent->DataSource = new clsDBConnection1();
            $this->s_SoloParent->ds = & $this->s_SoloParent->DataSource;
            $this->s_SoloParent->DataSource->SQL = "SELECT * \n" .
"FROM lut_yes {SQL_Where} {SQL_OrderBy}";
            list($this->s_SoloParent->BoundColumn, $this->s_SoloParent->TextColumn, $this->s_SoloParent->DBFormat) = array("Ans", "Ans", "");
            $this->s_SoloParent->Required = true;
        }
    }
//End Class_Initialize Event

//Validate Method @14-7772969C
    function Validate()
    {
        global $CCSLocales;
        $Validation = true;
        ...
        $this->DataSource->s_SoloParent->SetValue($this->s_SoloParent->GetValue());
        if(CCDLookUp("COUNT(*)", "", "=" . $this->DataSource->ToSQL($this->DataSource->s_SoloParent->GetDBValue(), $this->DataSource->s_SoloParent->DataType) . $Where, $this->DataSource) > 0)
            $this->s_SoloParent->Errors->addError($CCSLocales->GetText("CCS_UniqueValue", "s_SoloParent"));
        $Validation = ($this->s_employee_OfficeID->Validate() && $Validation);
        $Validation = ($this->s_SoloParent->Validate() && $Validation);
        $this->CCSEventResult = CCGetEvent($this->CCSEvents, "OnValidate", $this);
        $Validation =  $Validation && ($this->s_employee_OfficeID->Errors->Count() == 0);
        $Validation =  $Validation && ($this->s_SoloParent->Errors->Count() == 0);
        return (($this->Errors->Count() == 0) && $Validation);
    }
//End Validate Method

//CheckErrors Method @14-4C8C2FBF
    function CheckErrors()
    {
        $errors = false;
        $errors = ($errors || $this->s_employee_OfficeID->Errors->Count());
        $errors = ($errors || $this->s_SoloParent->Errors->Count());
        $errors = ($errors || $this->Errors->Count());
        return $errors;
    }
//End CheckErrors Method
...
//Operation Method @14-38785273
    function Operation()
    {
        ...
        $Redirect = "Query_SoloParent.php";
        if($this->Validate()) {
            if($this->PressedButton == "Button_DoSearch") {
                $Redirect = "Query_SoloParent.php" . "?" . CCMergeQueryStrings(CCGetQueryString("Form", array("Button_DoSearch", "Button_DoSearch_x", "Button_DoSearch_y")));
...
//Show Method @14-346DA685
    function Show()
    {
        ...
        if($this->FormSubmitted || $this->CheckErrors()) {
            ...
            $Error = ComposeStrings($Error, $this->s_SoloParent->Errors->ToString());
        ...
        $this->s_SoloParent->Show();
        $Tpl->parse();
        $Tpl->block_path = $ParentPath;
    }
//End Show Method

} //End departmentoffice_employee Class @14-FCB6E20C



//Initialize Page @1-60FD4892
// Variables
$FileName = "";
$Redirect = "";
$Tpl = "";
...
$FileName = FileName;
$Redirect = "";
$TemplateFileName = "Query_SoloParent.html";
$BlockToParse = "main";
...
//Include events file @1-3383880B
include_once("./Query_SoloParent_events.php");
...
$Report_Print->Page = "Query_SoloParent.php";

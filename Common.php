...
//Connection Settings @0-EF178B23
$CCConnectionSettings = array (
   
    "Connection1" => array(
        "Type" => "MySQL",
        "DBLib" => "MySQL",
        "Database" => "contractualjo",
        "Host" => "localhost",
        "Port" => "",
        "User" => "root",
        "Password" => "",
...
//Initialize Common Variables @0-73F2B1F1
...
//CCLoginUser @0-50BDB735
function CCLoginUser($login, $password)
{
    CCLogoutUser();
    $db = new clsDBConnection1();
    $SQL = "SELECT UserID, UserLevel, Password FROM users WHERE UserName=" . $db->ToSQL($login, ccsText) . " AND Password=" . $db->ToSQL($password, ccsText);
    $db->query($SQL);
    $Result = $db->next_record();
    if ($Result) {
        CCSetSession("UserID", $db->f("UserID"));
        CCSetSession("UserLogin", $login);
        CCSetSession("GroupID", $db->f("UserLevel"));
    }
    return $Result;
}
//End CCLoginUser
